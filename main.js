...
Elm.Seaport.make = function (_elm) {
   ...
   var seaportStr = function (seaport) {
      return function () {
         switch (seaport.ctor)
         {case "Just":
            return $String.concat(_L.fromArray([seaport._0.code
                                               ,", "
                                               ,seaport._0.name
                                               ,", "
                                               ,seaport._0.country]));
   ...
   model) {
      return function () {
         ...
         {case "EmptyAction":
            ...
            case "PickupEnter":
            return function () {
                 var seaport = A2(selectedPort,
                 model.counter,
                 model.ports);
                 var s = A2($Maybe.withDefault,
                 A3(newSeaport,"","",""),
                 seaport);
                 var code = s.code;
                 return {ctor: "_Tuple2"
      ...
      return function () {
         ...
         {case "From":
            ...
            return function () {
                 ...
                 model.tariff.tariffs,
                 model.tariff.filter,
                 model.tariff.setFilter,
                 model.tariff.filterTariffs)),
                 tariff = $._0,
                 fx = $._1;
                 return {ctor: "_Tuple2"
                        ,_0: A3(Model,
...
Elm.Tariff = Elm.Tariff || {};
Elm.Tariff.make = function (_elm) {
   "use strict";
   _elm.Tariff = _elm.Tariff || {};
   if (_elm.Tariff.values)
   ...
   var radioFilterUniversal = F5(function (address,
   setFilter,
   filter$,
   tag,
   style) {
      return function () {
         var isChecked = A2($Set.member,
         style,
         filter$);
         return A2($Html.li,
         _L.fromArray([]),
         ...
         _L.fromArray([A2($Html.input,
                      _L.fromArray([$Html$Attributes.type$("checkbox")
                                   ...
                                   function ($) {
                                      return $Signal.message(address)(tag(style)($));
                                   })]),
                      _L.fromArray([]))
                      ,A2($Html.span,
                      _L.fromArray([]),
                      _L.fromArray([$Html.text(style)]))]))]));
      }();
   });
   var listFilterUniversal = F4(function (address,
   model,
   filter,
   tag) {
      return A2($List.map,
      function (x) {
         return A5(radioFilterUniversal,
         address,
         model.setFilter,
         filter,
         tag,
   ...
   var checkbox = F5(function (address,
   model,
   filter,
   tag,
   title) {
         ...
         {case false:
            return A2($Html.div,
              ...
              _L.fromArray([A2($Html.p,
                           ...
                           ,A2($Html.ul,
                           _L.fromArray([$Html$Attributes.$class("filter owners")]),
                           A4(listFilterUniversal,
                           address,
                           model,
                           filter,
                           tag))]));
            case true: return A2($Html.div,
              _L.fromArray([$Html$Attributes.$class("hidde")]),
              _L.fromArray([]));}
         _U.badCase($moduleName,
   ...
   var filterTariff = F2(function (filter,
   tariffs) {
      return A2($List.filter,
      function (x) {
         return A2(byContainers,
         x,
         filter.containers);
      },
      A2($List.filter,
   ...
   var FilterByStatus = F2(function (a,
   b) {
      return {ctor: "FilterByStatus"
             ,_0: a
             ,_1: b};
   });
   var FilterByContainers = F2(function (a,
   b) {
      return {ctor: "FilterByContainers"
             ,_0: a
             ,_1: b};
   });
   var FilterByOwners = F2(function (a,
   b) {
      return {ctor: "FilterByOwners"
             ,_0: a
             ,_1: b};
   });
   var view$ = F2(function (address,
   model) {
      ...
      _L.fromArray([A5(checkbox,
                   address,
                   model,
                   model.filter.containers,
                   FilterByContainers,
                   "Containers")
                   ,A5(checkbox,
                   address,
         ...
         _L.fromArray([A2($Html.div,
                      ...
                      _L.fromArray([A2($Html.div,
                                   ...
                                   _L.fromArray([A2($Html.span,
                                                ...
                                                ,A2($Html.span,
                                                _L.fromArray([]),
                                                _L.fromArray([$Html.text(t.owner)]))]))]))
                                   ,A2($Html.div,
                                   _L.fromArray([$Html$Attributes.$class("pure-u-6-24")]),
                                   _L.fromArray([A2($Html.p,
                                   _L.fromArray([]),
   ...
   var decodePorts = function () {
      var tariff = A9($Json$Decode.object8,
      Tariff,
      A2($Json$Decode._op[":="],
      "company",
      $Json$Decode.string),
      ...
      return A2($Json$Decode._op[":="],
      "tariffs",
      $Json$Decode.list(tariff));
   }();
   var getListTariff = F2(function (pol,
   pod) {
      return $Effects.task($Task.map(NewList)($Task.toMaybe(A2($Http.get,
      decodePorts,
      A2(portUrl,pol,pod)))));
      ...
      return function () {
         ...
         {case "FilterByContainers":
            return function () {
                 ...
                 var setFilter = A3(Filter,
                 m.setFilter.owners,
                 set,
                 m.setFilter.status);
                 var filterTariffs = A2(filterTariff,
                 setFilter,
                 m.tariffs);
                 return {ctor: "_Tuple2"
                        ,_0: A6(Model,
            ...
            case "NewList":
            return function () {
                 var tariffs = A2($Maybe.withDefault,
                 m.tariffs,
                 a._0);
                 var listContainers = A2($List.map,
                 function (_) {
                    return _.container;
                 },
                 tariffs);
                 var filterContainers = $Set.fromList(listContainers);
                 var listOwners = A2($List.map,
                 function (_) {
                    return _.owner;
                 },
                 tariffs);
                 var filterOwners = $Set.fromList(listOwners);
                 var listStatus = A2($List.map,
                 function (_) {
                    return _.status;
                 },
                 tariffs);
                 var filterStatus = $Set.fromList(listStatus);
                 var filter = {_: {}
